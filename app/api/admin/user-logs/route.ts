...
export async function GET(req: NextRequest) {
  ...
  if (!targetEmail) return NextResponse.json({ error: "email param required" }, { status: 400 });
  ...
  const [txDays, vsDays] = await Promise.all([
    db.collection("transactions").aggregate([
      { $match: { userEmail: targetEmail, createdAt: { $gte: since } } },
  ...
  return NextResponse.json({ email: targetEmail, days });
}
